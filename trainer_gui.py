...
class TrainingMonitor(QMainWindow):
    ...
    def extract_symbols_from_ticker_data(self, ticker_data) -> List[str]:
        ...
        for symbol, payload in iterable:
            if not symbol or symbol in seen:
                # Если попалась вложенная структура, пытаемся её распаковать
                if isinstance(payload, list):
                    for entry in payload:
                        if isinstance(entry, dict):
                            nested_symbol = entry.get('symbol')
                            if nested_symbol and nested_symbol not in seen:
                                seen.add(nested_symbol)
                ...
                try:
                    ...
                    if instruments:
                        ...
                        for instrument in instruments:
                            ...
                            # Проверяем что инструмент активен и символ не пустой
    ...
    def _collect_symbols_from_cache(self) -> Tuple[List[str], List[str]]:
        ...
        if hasattr(self.ticker_loader, 'get_all_symbols'):
            raw_symbols.update(self.ticker_loader.get_all_symbols())
        else:
    ...
    def load_symbols(self):
        ...
        try:
            ...
            if self.ticker_loader:
                # Обновляем внутренние структуры загрузчика, чтобы гарантировать актуальные данные
                try:
                    self.ticker_loader.load_tickers_data()
